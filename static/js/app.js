...
async function loadBlocklistSuggestions(type) {
    ...
    try {
        ...
        container.innerHTML = summaryHtml + suggestions.map((item, index) => {
            ...
            // Icon for company link
            const arrowIcon = type === 'company' ? `
                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-gray-500 hover:text-blue-400 cursor-pointer ml-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" onclick="window.open('${itemUrl}', '_blank'); event.stopPropagation();" title="Open LinkedIn Page">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex-1 min-w-0 mr-4">
                            <div class="flex items-center">
                                <span class="text-sm font-bold text-white truncate group-hover:text-blue-100">${escapeHtml(displayValue)}</span>
                                ${arrowIcon}
                            </div>
